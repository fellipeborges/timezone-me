...
- Drag the grip on a city row to reorder (ArrowUp / ArrowDown on the grip also works)
...
- The hour grid is aligned to the first timezone in the list (reordering the top row changes alignment)
- Timezones live in the URL in list order, so you can share a view. People names are stored in `n` (one slot per row; use `%0A` for line breaks):
